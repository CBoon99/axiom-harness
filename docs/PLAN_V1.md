...
- **Master untouched:** `Axiom Harness Master/` (now on disk `Axiom Harness-Master - Dont Touch/`) stays frozen at 2026-09-24 05:39 until you approve a promotion; all P0-P5 work lands in `Axiom-Harness-Staging/` on paper but on disk at `Axiom Harness-Master - Dont Touch/Staging/` and git remote `axiom-harness` (lowercase, [https://github.com/CBoon99/axiom-harness](https://github.com/CBoon99/axiom-harness)) — aliases documented here to avoid reviewer confusion; `STAGING_ROOT` resolves under `.../Axiom Harness-Master - Dont Touch/Staging`.
