...
| **ECA CURRENT CANONICAL** | `Axiom Evidence Layer/ECA` | `ECA/plugin/` (`__init__.py`, `config.py`, `provider/anthropic.py`, `receipt.py`) | `bf7329c836d8b820` (historical `sim/telescope.py` hash, plugin `__init__` hash `ef3573e` not yet separately hashed) | ECA owns `eca_config.json` (Harness competing copy `eca_config.json` deleted `rm` 2026-09-24) | UNKNOWN (ECA/spec `1.2`) | `ECA Plugin LOAD/CONFIGURE/FREEZE/RUN/RECORD/SEAL` via `eca_config.json` + `ProviderAdapter` | **UPSTREAM-BLOCKED** — do not certify ECA; Harness records `ECA_IMPLEMENTATION_HASH` + `ECA_CONFIG_HASH` only |
| **ECA HISTORICAL (BLOCKED ref)** | `Axiom Evidence Layer/ECA` | `ECA/sim/telescope.py` | `bf7329c836d8b820` | — | `sim/telescope.py` `L0→L4 GateOk` | `Telescope + GateOk + FALSIFY P2 + evidence_gate` | **HISTORICAL / BLOCKED — not current canonical; do not certify** |
