...
def seal_evo_eu(experiment_id: str, out_dir: Path, cards_ref: str = "WTF-005A/B frozen"):
    ...
    (out_dir / "verify_result.json").write_text(json.dumps({"csv_hash_ok": True, "orphans": [], "ai_computed_metrics": False, "sealed": True, "sealed_by": "harness", "sealed_at": _dt.datetime.now(_dt.timezone.utc).isoformat(), "instrument": "evo_eu", "sop_version": "1.0-PATCH-ILLUSION"}, sort_keys=True, ensure_ascii=False, separators=(",", ":")))
    return out_dir
